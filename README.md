...
## Ubiquitous language
   ...
   * Покупатель/Customer - человек, который просматривает и покупает товары в магазине
   * Добавление товара в корзину/Add product to shopping cart
   * Удаление товара из корзины/Delete product from shopping cart
   * Оформление заказа/Ordering - процесс бронирования товаров, чтобы в дальнейшем их оплатить
   ...
   * Товар/Product - объект, который может просматривать и приобретать покупатель
   ...
   * Курьерская служба/Доставка/Delivery Service - сервис, который отвечает за доставку товаров
   ...
   * Ожидает оплаты/Awaiting payment - заказ забронирован, но еще не оплачен
   * Оплачен/Order is paid - пользователь оплатил заказ
   * Отменен/Order cancelled - заказ отменен
   * Корзина/Shopping basket - место, где хранятся товары, которые пользователь хочет купить
   * Платежная система/Payment Service - сервис для произведения оплаты заказов
